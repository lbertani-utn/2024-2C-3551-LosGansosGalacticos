...
namespace TGC.MonoGame.TP.Collisions
{
    ...
    public static class BoundingVolumesExtensions
    {
        ...
        {
            BoundingBox box = CreateAABBFrom(model);
            ...
            return new BoundingCylinder(center, maxRadius, box.Max.Y - center.Y);
        }

        /// <summary>
        ///     Creates an array of <see cref="BoundingBox">BoundingBox</see> from a Model, using all its sub-meshes.
        /// </summary>
        /// <param name="model">The model to create the boxes</param>
        /// <returns>The <see cref="BoundingBox">BoundingBox</see> array that encloses the vertices from the model</returns>
        public static BoundingBox[] CreateAABBArrayFrom(Model model)
        {
            ...
            for (int index = 0; index < meshes.Count; index++)
            {
                ...
                boxes[index] = new BoundingBox(minPoint, maxPoint);
            }
            return boxes;
        }
